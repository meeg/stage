...
int convertHexArrayToDec(char* hexArray, long* decResult){
	...
	//It expects groups of 4 digits separated by spaces.
	...
	//int numChars = lenArray.length() + 1;	//on ppa-pc83230-l this needed the +1, but on ec-clnrm the +1 leads to an error?
	int numChars = lenArray.length();
	//printf("numChars=%d\n", numChars);
	int numGroups;// = (numChars-1)/hexGroupLen;
	
	if ( ( numChars == 4 ) | ( numChars == 5 )  ) numGroups = 1;			//4 hex + cr
	if ( ( numChars == 10 ) | (numChars == 9) ) numGroups = 2;		//8 hex + space + cr;
	if ( ( numChars == 15 ) | ( numChars == 14 ) ) numGroups = 3;		//12 hex + 2 spc + cr
	//printf("hexArray=%s<end>\n", hexArray);
	//printf("numGroups in convertHexArrayToDec()= %d\n", numGroups);
	//printf("in convertHexArrayToDec(): numGroups=%d, numChars=%d\n", numGroups, numChars);
/*	if (  ( numChars != hexGroupLen+1 ) & 
	...
	//printf("	decValue[ 1 ] = %d\n", decValue[ 1 ]);
	if ( numGroups == 1 ){
		decValue[ 1 ] = strtol(hexArray, &pEnd, 16);
		//printf("	decValue[ 1 ] = %d\n", decValue[ 1 ]);		
	}else if ( numGroups == 2 ){
		decValue[ 0 ] = strtol(hexArray, &pEnd, 16);
		decValue[ 1 ] = strtol(pEnd, &pEnd, 16);
		//printf("	decValue[ 1 ] = %d\n", decValue[ 1 ]);		
	}else if ( numGroups == 3 ){
		decValue[ 0 ] = strtol(hexArray, &pEnd, 16);
		decValue[ 1 ] = strtol(pEnd, &pEnd, 16);
		decValue[ 2 ] = strtol(pEnd, &pEnd, 16);
	}
		
	
/*	for (int extraWords = 1; extraWords < numGroups; extraWords++){
		//decValue[ 1 ] = (int) strtol(pEnd, &pEnd, 16);
		int testVal = (int) strtol(pEnd, &pEnd, 16);
		//printf("extracting extra groups\n");
		printf("testVal = %d\n", testVal);	
	...
	if ( numGroups == 1 ){
		//printf("%d\n", decValue[ 0 ]);
		//printf("returned:%d\n", decValue[0] );
		long valToSendBack = decValue[1];
		//printf("dec result to return:%ld\n", valToSendBack);
		*decResult = decValue[1];
	...
	if ( numGroups == 2 ){
		...
		valToSendBack = decValue[0]*65536 + decValue[1];
	
		*decResult = valToSendBack;
		//printf("sending back: %d\n", decResult);
		return 0;
	}
...
int getHexArrayFromReply( char* inputArray, char* hexResult, int numWordsToGet){
	...
	if ( numWordsToGet == 2 ){
		...
		hexResult[ 9 ] = 0;		
		//printf("getHexArrayFromReply() parsed a PSW of %s\n", hexResult);
		return 0;	
	}
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
